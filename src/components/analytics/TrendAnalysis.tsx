...
export function TrendAnalysis() {
  ...
  const { data: trendData, isLoading } = useQuery({
    ...
    queryFn: async () => {
      ...
      const { data: invoices } = await supabase.from("invoices").select("total, status, created_at").gte("created_at", lastYearMonths[0].start);
      ...
      const yoyComparison = currentYearMonths.map(({ month, fullMonth, start, end }, index) => {
        ...
        const currentRevenue = invoices?.filter((inv) => inv.created_at >= start && inv.created_at <= end && inv.status === "paid").reduce((sum, inv) => sum + (Number(inv.total) || 0), 0) || 0;
        const lastYearRevenue = invoices?.filter((inv) => inv.created_at >= lastYearPeriod.start && inv.created_at <= lastYearPeriod.end && inv.status === "paid").reduce((sum, inv) => sum + (Number(inv.total) || 0), 0) || 0;
      ...
      const momChanges = currentYearMonths.slice(1).map((curr, index) => {
        ...
        const currRevenue = invoices?.filter((inv) => inv.created_at >= curr.start && inv.created_at <= curr.end && inv.status === "paid").reduce((sum, inv) => sum + (Number(inv.total) || 0), 0) || 0;
        const prevRevenue = invoices?.filter((inv) => inv.created_at >= prev.start && inv.created_at <= prev.end && inv.status === "paid").reduce((sum, inv) => sum + (Number(inv.total) || 0), 0) || 0;
  ...
  if (isLoading) return <div className="space-y-6"><div className="grid gap-4 md:grid-cols-3">{[1, 2, 3].map((i) => (<Card key={i} className="glass"><CardHeader className="pb-2"><Skeleton className="h-4 w-24" /></CardHeader><CardContent><Skeleton className="h-8 w-32" /></CardContent></Card>))}</div></div>;
  ...
  return (
    <div className="space-y-6">
      ...
      <Card className="glass border-border"><CardHeader><CardTitle className="flex items-center gap-2"><Calendar className="h-5 w-5 text-primary" />Year-over-Year Revenue</CardTitle></CardHeader><CardContent><div className="h-[300px]"><ResponsiveContainer width="100%" height="100%"><BarChart data={trendData?.yoyComparison}><CartesianGrid strokeDasharray="3 3" className="stroke-muted" /><XAxis dataKey="month" tick={{ fill: 'hsl(var(--muted-foreground))' }} /><YAxis tick={{ fill: 'hsl(var(--muted-foreground))' }} tickFormatter={(v) => `${v}K`} /><Tooltip contentStyle={{ backgroundColor: 'hsl(var(--background))', border: '1px solid hsl(var(--border))', borderRadius: '8px', color: 'hsl(var(--popover-foreground))',}} formatter={(value: number) => [`${formatCurrency(value * 1000)}`, '']} /><Legend /><Bar dataKey="currentYear" name="This Year" fill="hsl(var(--primary))" radius={[4, 4, 0, 0]} /><Bar dataKey="lastYear" name="Last Year" fill="hsl(var(--muted-foreground))" radius={[4, 4, 0, 0]} /></BarChart></ResponsiveContainer></div></CardContent></Card>

      <div className="grid gap-6 lg:grid-cols-2">
        <Card className="glass border-border"><CardHeader><CardTitle className="flex items-center gap-2"><TrendingUp className="h-5 w-5 text-primary" />Month-over-Month</CardTitle></CardHeader><CardContent><div className="h-[250px]"><ResponsiveContainer width="100%" height="100%"><LineChart data={trendData?.momChanges}><CartesianGrid strokeDasharray="3 3" className="stroke-muted" /><XAxis dataKey="month" tick={{ fill: 'hsl(var(--muted-foreground))' }} /><YAxis tick={{ fill: 'hsl(var(--muted-foreground))' }} tickFormatter={(v) => `${v}K`} /><Tooltip contentStyle={{ backgroundColor: 'hsl(var(--background))', border: '1px solid hsl(var(--border))', borderRadius: '8px', color: 'hsl(var(--popover-foreground))',}} formatter={(value: number) => [`${formatCurrency(value * 1000)}`, 'Revenue']} /><Line type="monotone" dataKey="revenue" stroke="hsl(var(--primary))" strokeWidth={3} dot={{ fill: 'hsl(var(--primary))', strokeWidth: 2 }} /></LineChart></ResponsiveContainer></div></CardContent></Card>
        <Card className="glass border-border"><CardHeader><CardTitle className="flex items-center gap-2"><BarChart3 className="h-5 w-5 text-primary" />Win/Loss Rate</CardTitle></CardHeader><CardContent><div className="h-[250px]"><ResponsiveContainer width="100%" height="100%"><LineChart data={trendData?.conversionTrends}><CartesianGrid strokeDasharray="3 3" className="stroke-muted" /><XAxis dataKey="month" tick={{ fill: 'hsl(var(--muted-foreground))' }} /><YAxis tick={{ fill: 'hsl(var(--muted-foreground))' }} tickFormatter={(v) => `${v}%`} /><Tooltip contentStyle={{ backgroundColor: 'hsl(var(--background))', border: '1px solid hsl(var(--border))', borderRadius: '8px', color: 'hsl(var(--popover-foreground))',}} formatter={(value: number) => [`${value.toFixed(1)}%`, '']} /><Legend /><Line type="monotone" dataKey="winRate" name="Win Rate" stroke="hsl(var(--primary))" strokeWidth={2} /><Line type="monotone" dataKey="lossRate" name="Loss Rate" stroke="hsl(var(--destructive))" strokeWidth={2} /></LineChart></ResponsiveContainer></div></CardContent></Card>
      </div>
